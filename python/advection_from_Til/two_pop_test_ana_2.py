...
n_t       = 20
n_m       = 1
x         = logspace(log10(0.1),log10(1e4),n_r)*AU
...
T             = 120 * (x/AU)**(-3./7) #( (0.05**0.25*T_star * (x/R_star)**-0.5)**4 + 1e4)**0.25
T             = tile(T,[n_t,1])
...
#
profile     = maximum(M_disk*20/(2*pi*R_c**2)*(R_c/x)*exp(-x/R_c),1e-100)
...
#
plotter(x=x/AU,data=SOL,times=TI/year,xlog=1,ylog=1,xlim=[0.1,100],ylim=[1e-5, 1e5],xlabel='r [AU]',i_start=0,ylabel='$\Sigma_d$ [g cm $^{-2}$]')	
#
# export the data
#
dirname = 'data'
