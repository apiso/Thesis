...
\textbf{Passive disk.} A passive disk is only irradiated by the central star and does not experience accretion heating. We adopt a minimum mass solar nebula (MMSN) disk model for a passive disk similar to the prescription of \citet{chiang10}. The gas surface density and midplane temperature are
\begin{subeqnarray}
\label{eq:disk}
...
\begin{equation}
\label{eq:activeT}
T_{\rm d}^4 = T_{\rm d, acc}^4 + T_{\rm d, pas}^4,
\end{equation}
where to avoid notation confusion $T_{\rm d, pas}=T_{\rm d}$ from Equation (\ref{eq:diskT}), the temperature profile for an active disk. We can then easily determine  $c_{\rm d}$ and $H_{\rm d}$ from Equation (\ref{eq:cdHd}), as well as the viscosity $\nu$ from Equation (\ref{eq:nu}) for a given $\alpha$. For consistency, we choose  $\alpha=0.01$ as in the previous case. Finally, we determine $\Sigma_{\rm d}$ from Equation (\ref{eq:Mdot}), where we choose $\dot{M}=10^{-8} M_{\odot}$ yr$^{-1}$ based on disk observations (e.g., \citealt{andrews10}). We note that $\Sigma_{\rm d}$ is no longer constant when stellar irradiation is taken into account in the disk evolution. 
...
\item Our model finds that the C/O ratio is enhanced compared to the stellar value throughput most of the disk, with the C/O ratio reaching unity between the CO$_2$ and CO snowlines. This is consistent with observations of C/O ratios in some exoplanet atmospheres. 
